...
export class VideoExtractorService {
  ...
  constructor(
    ...
    if (!this.apiUrl) {
      this.logger.warn(
        '⚠️ [VIDEO_EXTRACTOR] VIDEO_EXTRACTOR_API_URL no configurada - el servicio no estará disponible',
      );
    } else {
      this.logger.log(
        '✅ [VIDEO_EXTRACTOR] Servicio inicializado correctamente',
      );
   ...
   */
  async startThumbnailExtraction(
    ...
    this.logger.log(
      `🎬 [VIDEO_EXTRACTOR] Callback URL: ${request.callbackUrl}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `🎬 [VIDEO_EXTRACTOR] Payload: ${JSON.stringify(payload, null, 2)}`,
      );
      ...
      this.logger.log(
        `✅ [VIDEO_EXTRACTOR] Extracción iniciada exitosamente en ${processingTime}ms`,
      );
      this.logger.log(`🎬 [VIDEO_EXTRACTOR] Task ID: ${taskId}`);
      this.logger.log(
        `📊 [VIDEO_EXTRACTOR] Duración estimada: ${data.duration}s, Thumbnails estimados: ${data.total_thumbnails}`,
      );
      ...
      const processingTime = Date.now() - startTime;
      this.logger.error(
        `❌ [VIDEO_EXTRACTOR] Error iniciando extracción después de ${processingTime}ms: ${error.message}`,
      );

      if (error.response?.data) {
        this.logger.error(
          `❌ [VIDEO_EXTRACTOR] Detalles del error: ${JSON.stringify(error.response.data, null, 2)}`,
        );
      }

      throw new Error(
        `Error iniciando extracción de thumbnails: ${error.message}`,
      );
    }
  }
}
